...
{
	u64 m_uiCaseID;
	u32 m_uiHeaderPrefixLength;
	u32 m_uiLockFirstPersonAccountID;
...
{
	void DECLARE_BYTESWAP_DATADESC() { return; }
...
//-----------------------------------------------------------------------------
typedef void (*pfnDemoCustomDataCallback)(u8* pData, size_t iSize);
...
public:
	...
	virtual void				GetViewAngles(Vector3<float>& va) = 0;
	// Set current view orientation from va
	virtual void				SetViewAngles(Vector3<float>& va) = 0;
	...
	virtual void SetVoiceCasterID(u32 casterID) = 0;
#endif
	...
	virtual void		DebugDrawPhysCollide(const CPhysCollide* pCollide, IMaterial* pMaterial, const void* transform, const color32& color) = 0;
	...
	virtual u32			OnStorageDeviceAttached(int iController) = 0;
	...
	// For non-split screen games this will always be zero
		...
		virtual ISPSharedMemory* GetSinglePlayerSharedMemorySpace(const char* szName, int ent_num = 32) = 0;
		...
		virtual void RegisterDemoCustomDataCallback(void* szCallbackSaveID, pfnDemoCustomDataCallback pCallback) = 0;
		...
		virtual void SubmitStatRecord(char const* szMapName, u32 uiBlobVersion, u32 uiBlobSize, const void* pvBlob) = 0;
		...
		virtual void* GetInputContext(EngineInputContextId_t id) = 0;
...
public:
	...
	virtual void			SetCrosshairAngle(const Vector3<float>& angle) = 0;
	...
	virtual bool			DispatchUserMessage(int msg_type, i32 nFlags, int size, const void* msg) = 0;	// New protobuf version
	...
	virtual void			SetDemoPlaybackHighlightXuid(u64 xuid, bool bLowlights) = 0;
	...
	virtual void			CacheFrustumData(const void* frustum, const void* aeraFrustums) = 0;
